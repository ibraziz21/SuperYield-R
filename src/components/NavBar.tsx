  ...
  return (
    <Link
      ...
      className={`rounded-full px-3 py-1 text-sm transition ${
        active ? 'bg-teal-600 text-white' : 'text-muted-foreground hover:text-foreground hover:bg-muted/60'
      }`}
    >
      {children}
    </Link>
...
export function Navbar() {
  ...
  return (
    <>
      {/* Top App Bar */}
      <header
        className={`sticky top-0 z-50 w-full border-b border-border/60 backdrop-blur-md supports-[backdrop-filter]:bg-white/55 dark:supports-[backdrop-filter]:bg-background/60 ${
          elevated ? 'shadow-[0_1px_0_0_rgba(0,0,0,0.02)]' : ''
        }`}
      >
        <div className="mx-auto flex h-14 w-full max-w-6xl items-center justify-between px-3 sm:px-4">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/" className="group inline-flex items-center gap-2">
              <div className="h-7 w-7 rounded-xl bg-gradient-to-br from-teal-500 to-cyan-500 shadow-sm ring-1 ring-black/5 transition-transform group-active:scale-95" />
              <span className="truncate text-lg font-extrabold tracking-tight">
                <span className="bg-gradient-to-r from-teal-600 via-cyan-600 to-teal-600 bg-clip-text text-transparent">SuperYield-R</span>
              </span>
            </Link>
        ...
        >
          <div className="flex h-14 items-center justify-between border-b px-3">
            <div className="inline-flex items-center gap-2">
              <div className="h-6 w-6 rounded-lg bg-gradient-to-br from-teal-500 to-cyan-500 ring-1 ring-black/5" />
              <span className="text-sm font-bold">SuperYield‑R</span>
            </div>
            <button
              onClick={() => setMobileOpen(false)}
        ...
        {tabs.map((t) => {
          ...
          return (
            <Link
              key={t.href}
              href={t.href}
              className={`flex flex-col items-center justify-center gap-0.5 rounded-xl py-2 text-[11px] font-medium ${
                active ? 'bg-teal-600 text-white' : 'text-muted-foreground hover:bg-muted/60 hover:text-foreground'
              }`}
